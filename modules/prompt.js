...
class Prompt {
  ...
  async display(channel) {
    ...
    await msg.reactions.array().forEach(reaction => reaction.users.array().filter((fuser) => fuser.id != client.user.id).forEach(ruser => reaction.remove(ruser))); // remove extra reactions before we're ready
    return msg;
  }
